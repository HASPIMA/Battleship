...
// Version modificada de miniwin basada en la version original: 0.2.1

/*
Modifcaciones hechas sobre la versión: 
  se añadió la posibilidad de colocar titulos en las ventanas emergentes
  se agregó el color naranaja
  se quitó la interacción del botón maximizar en windows
                    ...
                    int nFunsterStil)
{
    ...
    hWnd = CreateWindowEx (
      ...
      "[UN] Battleship by HASPIMA",           /* Title Text */
      WS_OVERLAPPEDWINDOW&~WS_MAXIMIZEBOX, /* type of window*/
...
void mensaje(std::string title,std::string msj) {
   MessageBox(hWnd, msj.c_str(), title.c_str(), MB_OK);
}

bool pregunta(std::string title,std::string msj) {
   return MessageBox(hWnd, msj.c_str(), title.c_str(), MB_OKCANCEL) == IDOK;
}
...
void mensaje(string msj,std::string title) {
   cerr << title << msj << endl;
}

bool pregunta(string msj,std::string title) {
   cerr << title << msj << endl;
   return false;
}
...
#error "Este programa no es compatible en esta plataforma"
